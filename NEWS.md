...
Documenter writes a `stable` and `vX.Y` copy of a site only when the docs build
runs at a release tag, and no scaffolded package had ever had one: TagBot tags
with the Actions app token unless a `DOCUMENTER_KEY` deploy key is set, and
GitHub raises no `push` event for a ref that token created.
`setup_checklist` now lists the deploy key, and the infrastructure docs carry a
`gh` recipe for adding it plus the details that break the deploy silently when
wrong.
