...
@testitem "AD harness" begin
    ...
    function build_registry()
        ...
        f2(θ, c) = sum(abs2, θ .- c)
        ref(f, θ, ctx) =
            DifferentiationInterface.gradient(f, AutoForwardDiff(), θ, ctx...)
        ...
        return (
            ...
            backends=() -> [
                ...
                (name="ReverseDiff", backend=AutoReverseDiff(compile=false)),
            ],
    ...
    @testset "broken bookkeeping marks a scenario broken" begin
        ...
        broken_reg = merge(reg, (broken_scenario_names=() -> ["centred"],))
    ...
    @testset "ad_backend_support_table renders declarations" begin
        ...
        @test occursin(
            "| Backend | Scenarios | Declared broken | Skipped |", tbl
        )
        ...
        declared = merge(
            ...
            (
                ...
                backend_broken_scenarios=() ->
                    Dict("ReverseDiff" => Set(["sum_squares"])),
                backend_skip_scenarios=() ->
                    Dict("ReverseDiff" => Set(["centred"])),
            ),
        )
    ...
    @testset "optional bookkeeping accessors default to empty" begin
        ...
        minimal_reg = (scenarios=reg.scenarios, backends=reg.backends)
