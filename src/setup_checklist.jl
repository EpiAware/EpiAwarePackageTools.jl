...
function _setup_checklist_steps(pkg::AbstractString, repo::AbstractString)
    return String[
        ...
        string("Add a `DOCUMENTER_KEY` deploy key to ", pkg, " so TagBot ",
            "pushes release tags over SSH. Generate the pair with `julia -e ",
            "'using DocumenterTools; DocumenterTools.genkeys(user=\"",
            ...
            "secret. Without it TagBot tags with the Actions app token: ",
            "GitHub raises no push event for that tag, so the versioned ",
            "release docs never build, and a tagged commit that touches a ",
            "workflow file is refused outright. Copy-pasteable `gh` recipe, ",
            "and the four details that silently break the deploy when they ",
            "are wrong: https://epiawarepackagetools.epiaware.org/dev/",
