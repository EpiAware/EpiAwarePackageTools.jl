...
function _setup_checklist_steps(pkg::AbstractString, repo::AbstractString)
    return String[
        string("Enable the repo on Codecov (https://app.codecov.io/gh/",
            repo, ") and add the `CODECOV_TOKEN` repo secret (Settings -> ",
            "Secrets and variables -> Actions)."),
        string("Enable GitHub Pages for the `gh-pages` branch (Settings -> ",
            "Pages) so the docs site deploys."),
        string("If using a custom docs subdomain (the `docs_subdomain` ",
            "input to `scaffold`/`update`), add a DNS CNAME for it and set ",
            "it as the custom domain in Settings -> Pages; the default ",
            "project-pages URL needs no DNS."),
        string("Protect the `main` branch (Settings -> Branches): require ",
            "a pull-request review and passing status checks before ",
            "merge."),
        string("Once ready to publish, register the package with the ",
