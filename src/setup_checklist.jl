...
function _setup_checklist_steps(pkg::AbstractString, repo::AbstractString)
    return String[
        string(
            "Enable ",
            pkg,
            " on Codecov (https://app.codecov.io/gh/",
            repo,
            ") and add the `CODECOV_TOKEN` repo secret (Settings -> ",
        ...
        string(
            "If ",
            pkg,
            " uses a custom docs subdomain (the ",
        ...
        string(
            "Protect ",
            pkg,
            "'s `main` branch (Settings -> Branches): ",
        ...
        string(
            "Once ",
            pkg,
            " is ready to publish, register it with the ",
...
function setup_checklist(
    ...
    inputs = scaffold_inputs(target_dir; package=package, repo=repo, org=org)
